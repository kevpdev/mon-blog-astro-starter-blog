...
export async function getRelatedPosts(
  ...
  const currentTags = (currentPost.data.tags || []).map((tag: string) => tag.toLowerCase().trim());
  ...
  allPosts.forEach((post) => {
    ...
    const postTags = (post.data.tags || []).map((tag: string) => tag.toLowerCase().trim());
  ...
  if (relatedPosts.length < limit) {
    ...
    const excludedSlugs = new Set([currentPost.slug, ...relatedPosts.map((post) => post.slug)]);
